...
def get_keywords_binaryArray(sentence, keywords):
    ...
    doc = nlp(sentence)
    # Iterates over the noun chunks, extracts the text of each noun and stores it in phrases 
    phrases = [chunk.text for chunk in doc.noun_chunks]
    for phrase in phrases:
        match, score = process.extractOne(phrase, symptom_list)
        # The score for the match is calculated and is inserted in keywords only if the score is > 80
        if score > 80:
